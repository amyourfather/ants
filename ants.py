...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while check_place and (not check_place.bees) and check_place.entrance is not hive:
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.max_range):
            if check_place.bees or check_place.entrance is hive:
                break
            check_place = check_place.entrance
        return random_or_none(check_place.bees)
        # END Problem 3 and 4
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range):
            if check_place.entrance == hive:
                return None
            check_place = check_place.entrance
        ori_place = self.place
        self.place = check_place
        temp = ThrowerAnt.nearest_bee(self, hive)
        self.place = ori_place
        return temp
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= amount:
            for elements in self.place.bees[:]:
                elements.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        if self.firstornot:
            QueenAnt.firstornot = False
            self.firstornot = True
    ...
    def action(self, colony):
        ...
        if self.firstornot:
            ...
            while pointer.exit:
                ...
                if pointer.ant:
                    if isinstance(pointer.ant, BodyguardAnt):
                        if isinstance(pointer.ant, TankAnt):
                            if not pointer.ant.Buffed_by_queen:
                                pointer.ant.damage *= 2
                                pointer.ant.Buffed_by_queen = True
                        if pointer.ant.contained_ant:
                            if not pointer.ant.contained_ant.Buffed_by_queen:
                                pointer.ant.contained_ant.damage *= 2
                                pointer.ant.contained_ant.Buffed_by_queen = True
                    else:
                        if not pointer.ant.Buffed_by_queen:
                            pointer.ant.damage *= 2
                            pointer.ant.Buffed_by_queen = True
...
def make_scare(action, bee):
    ...
    def new_action(col):
        nonlocal d
        """
        if bee.Correct_direction:
            bee.Correct_direction = False
        ent, exi = bee.place.entrance , bee.place.exit
        if not bee.Correct_direction:
            bee.place.exit = ent
            bee.place.entrance = exi
        action(col)
        ...
        if d == 0:
            #if not bee.Correct_direction:
            #    bee.place.entrance , bee.place.exit = bee.place.exit , bee.place.entrance
            #bee.Correct_direction = True
            bee.Correct_direction = True
...
def apply_effect(effect, bee, duration):
    ...
    if effect is make_scare and not bee.Been_scared:
        bee.Dur = duration
